...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
          let dynamicSvgNft, deployer, V3Aggregator, tokenCounter, transaction
          ...
          beforeEach(async function () {
              ...
              V3Aggregator = await ethers.getContract(
                  "MockV3Aggregator",
                  deployer
              )
          })
          ...
          describe("Constructor", () => {
              ...
              it("sets the aggregator addresses correctly", async function () {
                  ...
                  assert.equal(response, V3Aggregator.address)
              })
          })

          describe("mintNft", () => {
              beforeEach(async function () {
                  transaction = await dynamicSvgNft.mintNft(highValue)
              })
              it("emits an event with the token counter and the value associated", async () => {
                  expect(transaction).to.emit(dynamicSvgNft, "CreatedNFT")
              })
          ...
          describe("tokenURI", () => {
              it("checks if the tokenId exists", async () => {
